...
function trimPropertiesMutation(obj) {
  for (let i in obj) obj[i] = obj[i].trim()
  return obj
}
...
function findLargestInteger(integers) {
  ...
  for (let i of integers) {
    if (i.integer > winner) winner = i.integer
  }
  return winner
}
...
class Counter {
  ...
  countDown() {
    ...
    else if (this.count > 0) return this.count -= 1
    else return 0
  }
}
...
class Seasons {
   ...
   */
  next() {
    if (this.lastIndex < 3) this.lastIndex++
    else this.lastIndex = 0

    return this.seasons[this.lastIndex]
  }
}
...
class Car {
   ...
   */
  drive(distance) {
    const totalMiles = this.mileage * this.fuel
    ...
    if (remainingMiles < 0) {
      ...
      this.fuel -= distance / this.mileage
    }
